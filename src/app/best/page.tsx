...
import { PedalCard, StarRating, type Pedal } from "@/components/pedal-card";
...
const bestGuideSlugs = new Set(getBestTypeSlugs());
          ...
          return (
            <section key={typeId} className="mb-16">
              <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
                ...
                {bestGuideSlugs.has(typeInfo.slug) ? (
                  <Link
                    href={`/best/${typeInfo.slug}`}
                    className="text-sm text-orange-500 hover:text-orange-400 transition-colors"
                  >
                    Best {typeInfo.name} pedals guide →
                  </Link>
                ) : (
                  <Link
                    href={`/${typeId}`}
