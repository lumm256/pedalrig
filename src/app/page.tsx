...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-gray-100">
      {/* Hero */}
      <section className="relative px-6 py-28 text-center bg-gradient-to-b from-gray-900 to-gray-950">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center rounded-md border border-gray-600 px-6 py-3 text-base text-gray-200 hover:bg-gray-800 transition-colors"
            >
              Build Your Board
            </Link>
          </div>
      ...
      <section className="px-6 py-20 max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold mb-2 text-white">Guitar Pedal Types</h2>
        <p className="text-gray-400 mb-10">Every effect explained — from overdrive to reverb.</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {pedalTypes.map((type) => (
            <Link key={type.id} href={`/${type.slug}`} className="group">
              <Card className="bg-gray-900 border-gray-800 hover:border-orange-500 transition-colors h-full cursor-pointer">
                <CardContent className="p-5">
                  <div className="text-3xl mb-3">{type.icon}</div>
                  <h3 className="font-semibold text-white group-hover:text-orange-400 transition-colors mb-1">
                    {type.name}
                  </h3>
                  <p className="text-sm text-gray-400 leading-snug">{type.shortDescription}</p>
                </CardContent>
              </Card>
            </Link>
          ))}
      ...
      <section className="px-6 py-20 bg-gray-900">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold mb-2 text-white">Top Rated Guitar Pedals</h2>
          <p className="text-gray-400 mb-10">Highest-rated pedals trusted by guitarists worldwide.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {topPedals.map((pedal) => (
              <Card key={pedal.id} className="bg-gray-950 border-gray-800 flex flex-col">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    <CardTitle className="text-base font-semibold text-white leading-tight">
                      {pedal.name}
                    </CardTitle>
                    <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30 shrink-0 capitalize">
                      {pedal.type}
                    </Badge>
                  </div>
                ...
                <CardContent className="flex flex-col gap-3 flex-1">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2">
                    <span className="text-lg font-bold text-white">${pedal.price}</span>
                    <a
      ...
      <section className="px-6 py-20 max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold mb-2 text-white">Signal Chain Order</h2>
            ...
            {signalChain.map((step, i) => (
              <div key={step.type} className="flex items-center gap-1">
                <div className="flex flex-col items-center gap-1 px-2">
                  ...
                  <span className="text-xs text-gray-400 text-center w-14 leading-tight">{step.label}</span>
                </div>
        ...
        <div className="mt-8">
          <Link
            href="/pedal-order"
            className="inline-flex items-center justify-center rounded-md border border-orange-500 px-4 py-2 text-sm text-orange-400 hover:bg-orange-500/10 transition-colors"
          >
      ...
      {/* Bottom CTA */}
      <section className="px-6 py-24 bg-gradient-to-t from-gray-900 to-gray-950 text-center">
